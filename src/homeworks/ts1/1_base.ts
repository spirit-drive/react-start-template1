...
export const getTransformFromCss = (transformCssString: string): TransformNumbers => {
...
// функция принимает массив со значениями любого типа
export const getNumberedArray = <T>(arr: T[]): { value: T; number: number }[] =>
  arr.map((value, number) => ({ value, number }));

// функция принимает массив объектов { value, number }
...
export const transformCustomers = (customers: Customer[]): { [id: number]: Omit<Customer, 'id'> } => {
  return customers.reduce((acc: { [id: number]: Omit<Customer, 'id'> }, customer: Customer) => {
